            ...
            for (int i = enemies.length - 1; i >= 0; i--) {
                ...
                if (enemy.getLocation().distanceSquaredTo(src) <= radius) {
                    ...
                    if ((enemy.getType() == RobotType.ARCHON || enemy.getType() == RobotType.WATCHTOWER || enemy.getType() == RobotType.LABORATORY)) {
                        ...
                        if (enemy.getHealth() <= enemy.getType().getMaxHealth(enemy.getLevel()) * AnomalyType.FURY.sagePercentage) {
                            buildingsKilled++;
                        }
                    } else {
                        unitHP += enemy.getType().getMaxHealth(1);
                        if (enemy.getHealth() <= enemy.getType().getMaxHealth(1) * AnomalyType.CHARGE.sagePercentage) {
                            unitsKilled++;
                        }
                    }
                } else {
